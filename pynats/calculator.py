...
class Calculator():
    ...
    def compute(self,replication=None):
        """ Compute all statistics for all target processes and a given replication
        """
        self.compute_subset(self._statnames,replication=replication)

    def compute_subset(self,subset,replication=None):
        """ Compute a subset of statistics for all target processes and a given replication
        """
        ...
        for stat in pbar:
            try:
                s = [i for i, s in enumerate(self._statnames) if s == stat][0]
            except ValueError:
                print(f'Statistic {stat} not found. Skipping.')

            pbar.set_description(f'Processing [{self._name}: {self._statnames[s]}]')
            start_time = time.time()
            try:
                self._adjacency[s] = self._statistics[s].adjacency(self.dataset)
            except Exception as err:
                warnings.warn(f'Caught {type(err)} for statistic "{self._statnames[s]}": {err}')
                self._adjacency[s] = np.NaN
            self._proctimes[s] = time.time() - start_time
        pbar.close()
    ...
    def get_correlation_df(self,with_labels=False,debias=False,which_stat=['spearman'],flatten_kwargs={}):
        ...
        if with_labels:
            return mdf, edges.isna().sum(), self.getstatlabels()
        else:
            return mdf, edges.isna().sum()
    ...
    def get_correlation_df(self,with_labels=False,flatten_kwargs={},**kwargs):
        ...
        for calc in [c[0] for c in self.calculators.values]:
            ...
            if with_labels:
                ...
                try:
                    mlabels = mlabels | out[2]
                except TypeError:
                    mlabels.update(out[2])
            ...
            mdf = mdf.append(df2)
            nandf = nandf.append(pd.DataFrame(out[1],columns=[calc.name]).T)
...
class CorrelationFrame():
    ...
    def __init__(self,cf=None,flatten_kwargs={},**kwargs):
        ...
        if cf is not None:
            if isinstance(cf,CalculatorFrame) or isinstance(cf,Calculator):
                ...
                self._mdf, self._nandf, self._shapes, self._slabels, self._dlabels = cf.get_correlation_df(with_labels=True,flatten_kwargs=flatten_kwargs,**kwargs)
                self._name = cf.name
            else:
                self.merge(cf)
    ...
    def merge(self,other):
        ...
        self._nandf = self._nandf.append(other.nandf)
        try:
