...
def convert_mdf_to_ddf(df):
    ddf = pd.pivot_table(data=df.stack(dropna=False).reset_index(),index='Dataset',columns=['Type','Source statistic','Target statistic'],dropna=False).T.droplevel(0).reorder_levels([1,2,0])
    return ddf