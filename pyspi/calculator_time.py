...
class Calculator:
    ...
    def compute(self):
        """Compute all pairwise interactions with timing measurement.

        This method computes all SPIs in the calculator while measuring
        the execution time for each SPI. Timing data is stored in the
        spi_timings attribute and saved to time_spi.txt and time.txt files.
        """
        ...
        for spi in pbar:
            ...
            end_time = time.time()
            self.spi_timings[spi] = end_time - start_time
            
        pbar.close()
        
        # Save timing data after computation is complete
        self._save_timing_data()

    def _save_timing_data(self):
        """Save timing data to output files.

        Creates two files:
        - time_spi.txt: CSV format with SPI name and timing
        - time.txt: Just timing values, one per line
        """
        with open("time_spi.txt", "w") as f_spi, open("time.txt", "w") as f_time:
            for spi, elapsed in self.spi_timings.items():
                f_spi.write(f'{spi},{elapsed}\n')
                f_time.write(f'{elapsed}\n')
